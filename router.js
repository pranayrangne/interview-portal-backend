    ...
    if (username || email) {
        ...
        dbConnection.query(searchUserQuery, [username, email], (err, result) => {
            if (err) {
                ...
                if (result.length === 0) {
                    const insertUserQuery = `INSERT INTO users (_id, fullName, username, password, birthDate, sex, accountType, terms, email) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
router.post("/user", (req, res) => {
    ...
    res.header("Access-Control-Allow-Origin", "*");
    const fetchUserQuery = `SELECT * FROM users WHERE (username = '${username}' OR email = '${username}' ) AND password = 'sha1(${password})'`;
    dbConnection.query(fetchUserQuery, (err, result) => {
        if (err) {
